...
class AudioProcessing(object):
	...
	def __init__(self, freq, input_audio):
		self.sample_freq = freq
		self.audio_data = input_audio
	...
	def set_echo(self, delay):
		...
		output_audio = np.zeros(len(self.audio_data), dtype = int16)
	...
	def _set_stretch(self, factor, window_size, h):
		...
		# normalize (16bit)
		# div by zero workaround
		if (result.max() != 0):
			result = ((2 ** (16 - 4)) * result/result.max())
		else:
			result = ((2 ** (16 - 4)) * result)
